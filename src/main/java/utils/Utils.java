 ...
 * 
 * 
 * Used to wait for a element until it found without using thread
 * Sharfuddin Ahamed
 * 
 *
...
public class Utils {
	 ...
	 * @param addCartSuccessmessage - selektor elementa koji cekamo
	 * @param waitInterval - vreme koje ce driver da ceka da se element pojavi u DOM stablu
	 * @return WebElement selektovani element
	 */
	public static WebElement waitToBeClickable(WebDriver driver, WebElement addCartSuccessmessage, int waitInterval) {
		WebElement element = (new WebDriverWait(driver, waitInterval)).until(ExpectedConditions.elementToBeClickable(addCartSuccessmessage));
		return element;
	}
